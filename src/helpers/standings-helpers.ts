...
export function calculateStandings(matches: Match[]): GroupStanding[] {
	...
	// Calculate results
	matches.forEach((m) => {
		if (m.grp?.startsWith("Group ") && m.h && m.a) {
			const gName = m.grp;
			const homeTeam = m.h;
			const awayTeam = m.a;
			...
			if (m._scoreH !== undefined && m._scoreA !== undefined) {
				const sh =
					typeof m._scoreH === "string" ? parseInt(m._scoreH, 10) : m._scoreH;
				const sa =
					typeof m._scoreA === "string" ? parseInt(m._scoreA, 10) : m._scoreA;
			...
			if (
				played &&
				groupsData[gName][homeTeam] &&
				groupsData[gName][awayTeam]
			) {
	...
	const result: GroupStanding[] = Object.keys(groupsData)
		.sort()
		.map((gName) => {
