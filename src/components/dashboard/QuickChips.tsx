		...
		<section className="w-full max-w-[1180px] mx-auto px-6 mt-1">
			<div className="flex flex-col gap-2.5 border-t border-white/5 pt-4">
				...
				<div className="flex flex-wrap gap-x-2 gap-y-2 items-center text-xs pb-1">
					<span className="font-mono text-rose-400 uppercase tracking-[0.12em] mr-2 flex items-center gap-1.5">
						<svg
							...
							className="lucide lucide-heart text-rose-400"
						>
						...
						<DialogContent className="w-[calc(100%-2rem)] sm:max-w-2xl bg-[#0f0f0f]/95 border-white/10 text-white rounded-2xl backdrop-blur-xl p-0 overflow-hidden shadow-[0_24px_100px_rgba(0,0,0,0.9)] ring-1 ring-white/10 duration-400 ease-out">
							<DialogHeader className="p-6 pb-4 border-b border-white/5">
								<div>
									...
									<DialogDescription className="text-white/50 text-sm mt-1.5">
										Choose teams to add to your quick filters. You can select
										multiple.
									</DialogDescription>
								</div>
							</DialogHeader>
									...
									{Object.keys(FLAGS)
										...
										.map((t) => {
											...
											return (
												...
												>
													<span className="text-lg leading-none select-none drop-shadow-md">
														{FLAGS[t]}
													</span>
													<span className="text-[13px] font-semibold">{t}</span>
													{isFav && (
														<span className="ml-0.5 text-rose-400 text-[10px] drop-shadow-[0_0_4px_rgba(251,113,133,0.8)]">
															❤️
														</span>
													)}
												</button>
							...
							<DialogFooter className="px-6 py-5 border-t border-white/5 bg-black/20 flex flex-row items-center justify-between sm:justify-between w-full">
								...
								<button
									...
									className="w-auto px-8 py-2 bg-rose-400 text-white text-sm font-bold rounded-full transition-all duration-300 hover:bg-rose-500 active:scale-95 shadow-[0_0_15px_rgba(251,113,133,0.25)] hover:shadow-[0_0_20px_rgba(251,113,133,0.4)]"
								>
									Done
								</button>
							</DialogFooter>
				...
				<div className="flex flex-wrap gap-x-2 gap-y-2 items-center text-xs">
					<span className="font-mono text-muted-foreground/60 uppercase tracking-[0.12em] mr-2">
						Hosts
					</span>
					{hosts.map((h) => (
						<button
