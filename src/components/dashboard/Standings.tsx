		...
		<section className="px-6 py-8 max-w-[1180px] mx-auto w-full flex-1 animate-in fade-in-30 duration-300">
			<div className="flex flex-col gap-2 mb-8">
				...
				<p className="text-xs sm:text-sm text-muted-foreground">
					Live updated standings from Group A to Group L. Top two teams
					automatically qualify, along with the four best third-placed teams.
					Click a team to view their matches.
				</p>
			</div>
							...
							<table className="w-full text-left border-collapse">
								<thead>
									<tr className="border-b border-white/5 text-[10px] font-bold font-mono text-muted-foreground/60 uppercase tracking-wider">
										...
										<th className="py-2.5 px-4.5 text-center w-12 text-[#2DE89A]">
											PTS
										</th>
									</tr>
								</thead>
								<tbody className="divide-y divide-white/2">
									{groupData.standings.map((standing, index) => {
										...
										if (isTop2) positionBadgeColor = "text-[#2DE89A] font-bold";
											...
											positionBadgeColor = "text-blue-400 font-bold";
										...
										return (
											...
											>
												...
												<td className="py-3 px-4.5 flex items-center gap-2 min-w-0">
													<span
														className={`font-mono text-[10px] w-4.5 text-center flex-shrink-0 ${positionBadgeColor}`}
													>
														{index + 1}
													</span>
												...
												{/* GD */}
												<td
													className={`py-3 px-2 text-center font-mono font-semibold ${
														standing.gd > 0
															? "text-[#2DE89A]/80"
															: standing.gd < 0
																? "text-red-400/80"
