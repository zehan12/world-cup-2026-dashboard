	...
	return (
		<main className="flex-1 max-w-[1180px] w-full mx-auto px-6 mt-2">
			...
			<div className="font-mono text-xs text-muted-foreground/60 tracking-widest uppercase py-4 border-b border-white/5">
				{filteredMatchesCount} Match{filteredMatchesCount !== 1 ? "es" : ""}{" "}
				Shown
			</div>
			...
			<div className="flex flex-col gap-2 mt-4 pb-12">
				{days.length === 0 ? (
					<div className="text-center py-20 text-muted-foreground">
						<div className="font-bold text-2xl uppercase text-foreground mb-2">
							No Matches Found
						</div>
						...
						return (
							...
							>
								...
								<div className="sticky top-[162px] md:top-[112px] z-10 py-3 mb-3 flex items-baseline gap-3 bg-gradient-to-b from-background via-background/90 to-transparent">
									<span
										className={`font-bold text-2xl uppercase tracking-wider ${isToday ? "text-gold" : "text-foreground"}`}
									>
										{dayHelper.dow}
									</span>
									...
									{isToday ? (
										...
										<Badge
											variant="outline"
											className="text-muted-foreground/75 border-white/5 font-mono text-[10px] tracking-wider uppercase px-2 py-0.5 rounded-full bg-white/3"
										>
											{matchesOfDay.length} Game
											{matchesOfDay.length !== 1 ? "s" : ""}
										</Badge>
									)}
								</div>
