...
import { etTodayStr, startUTC } from "@/helpers/date-helpers";
...
export function useEspnScores(initialMatches: Match[]) {
	...
	const applyEvents = (eventsList: ProcessedEvent[]) => {
		setMatches((prevMatches) => {
			...
			eventsList.forEach((ev) => {
				...
				if (ev.state) {
					m._st =
						ev.state === "in" ? "live" : ev.state === "post" ? "ft" : "up";
				}
				m._detail = ev.state === "in" ? ev.detail : "";

				const hasScore =
					ev.hScore != null && ev.aScore != null && ev.state !== "pre";
				if (!m.h) {
					// knockout: resolve teams from feed
					if (
						eh &&
						ea &&
						!isPlaceholder(ev.hName) &&
						!isPlaceholder(ev.aName)
					) {
						m._th = eh;
						m._ta = ea;
					}
	...
	useEffect(() => {
		...
		const lastDate = INITIAL_MATCHES[INITIAL_MATCHES.length - 1].d.replace(
			/-/g,
			"",
		);
